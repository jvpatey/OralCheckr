...
1. Clone the [OralCheckr monorepo](https://github.com/jvpatey/OralCheckr) and open the API app:

   ```bash
   git clone https://github.com/jvpatey/OralCheckr.git
   cd OralCheckr/apps/api
   ```
...
The API is configured for deployment on **Render** with PostgreSQL. In the Render dashboard, set **Root Directory** to `apps/api` when using the [OralCheckr monorepo](https://github.com/jvpatey/OralCheckr). Build command: `npm install && npm run build:prod`. Start command: `npm start`.

## 🔗 Related

- Frontend app: [`apps/web`](../web) in this repository ([OralCheckr on GitHub](https://github.com/jvpatey/OralCheckr))
